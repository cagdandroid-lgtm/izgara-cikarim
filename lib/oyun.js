...
module.exports = function oyunuKur(io, ogretmenMi) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('kontrol', (_, cb) => {
      ...
      if (cb) cb(r.hata ? { hata: r.hata } : { dogru: r.dogru, kesinlesen: r.kesinlesen });
    });

    /* ASIL SORUYA cevap — puan, ilerleme ve ölçme kaydı burada */
    socket.on('cevap', ({ secim } = {}, cb) => {
      const sid = sidler.get(socket.id);
      if (!sid) return;
      const r = durum.cevap(sid, Number(secim));
      ...
      gunluk(`🧠 cevap: ${r.oyuncu.kod} ${r.oyuncu.ad} → ${r.dogru ? 'DOĞRU +' + r.puan : 'yanlış'}` +
        ` (${r.bulmaca.id}, ${r.deneme}. deneme, tablo ${r.tabloKullandi ? 'kullanıldı' : 'kullanılmadı'})`);

      if (cb) {
        cb({
          dogru: r.dogru, puan: r.puan, sira: r.sira, deneme: r.deneme,
          dogruIndeks: r.dogruIndeks, ilerledi: r.ilerledi, tamamlandi: r.tamamlandi,
          siradaki: r.siradaki
        });
      }
      if (r.oda) io.to(r.oda).emit('takimSonuc', { dogru: r.dogru, puan: r.puan, sira: r.sira, kimden: r.oyuncu.ad });
