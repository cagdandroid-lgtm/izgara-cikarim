...
module.exports = function oyunuKur(io, ogretmenMi) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('kontrol', (_, cb) => {
      ...
      if (r.ikili) {
        // ölçüm: takım cevabı her iki üyeye de kaydedilir (puan gibi)
        const deneme = r.takim.denemeler + (r.dogru ? 1 : 0);
        for (const uid of r.takim.uyeler) {
          const u = durum.oyuncular.get(uid);
          if (u) durum.olcum.yaz(durum, u, { sonuc: r.dogru ? 'dogru' : 'yanlis', sureMs: r.dogru ? r.sure : durum.gecenMs(), deneme });
        }
      ...
      const deneme = r.oyuncu.denemeler + (r.dogru ? 1 : 0);
      const sureMs = r.dogru ? r.sure : durum.gecenMs();
      if (r.dogru && durum.mod === 'birlikte') {
        // birlikte modunda puan tüm çevrimiçi sınıfa yazılır; ölçüm de öyle
        for (const p of durum.oyuncular.values()) {
          if (p.cevrimici) durum.olcum.yaz(durum, p, { sonuc: 'dogru', sureMs, deneme });
        }
      } else {
        durum.olcum.yaz(durum, r.oyuncu, { sonuc: r.dogru ? 'dogru' : 'yanlis', sureMs, deneme });
      }
    ...
    if (ogretmen) {
      ...
      T('t:yayinla', () => {
        ...
        gunluk(`🎬 oturum açıldı → grup ${durum.grup} isim kartları yayınlandı`);
