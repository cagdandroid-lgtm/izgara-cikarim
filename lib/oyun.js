...
const panelOlaylari = require('./panelOlaylari');
...
module.exports = function oyunuKur(io, ogretmenMi) {
  ...
  io.on('connection', (socket) => {
    ...
    /* PAS GEÇ (yalnız bireysel, 90 sn takılma sonrası): 0 puan, kayda sonuc="atlandi" */
    socket.on('pasGec', (_, cb) => {
      const sid = sidler.get(socket.id);
      if (!sid) return;
      const r = durum.pasGec(sid);
      if (r.hata) { if (cb) cb({ hata: r.hata }); return; }
      durum.olcum.yaz(durum, r.oyuncu, {
        sonuc: 'atlandi', sureMs: r.sureMs, deneme: 1, tabloKullandi: r.tabloKullandi
      });
      gunluk(`⏭ pas geçti: ${r.oyuncu.kod} ${r.oyuncu.ad} (${r.bulmaca.id}, ${Math.round(r.sureMs / 1000)} sn) → 0 puan`);
      if (cb) cb({ ok: true, dogruAtama: r.dogruAtama, puan: 0 });
      yayinla();
    });
    ...
    socket.on('cevap', ({ atamalar } = {}, cb) => {
      ...
      if (cb) {
        ...
        cb({
          dogru: r.dogru, puan: r.puan, sira: r.sira, tekrar: !!r.tekrar,
    ...
    if (ogretmen) {
      ...
      /* SORU ATLAMA (yalnız senkron): önceki · sonraki · belirli soru */
      T('t:soruAtla', (v) => {
        const hedef = v.hedef === 'onceki' || v.hedef === 'sonraki' ? v.hedef : Number(v.hedef);
        const r = durum.soruAtla(hedef);
        if (r.hata) return r;
        gunluk(`↔️ soru atlandı (${v.hedef}) → ${r.bulmaca.id} · ${durum.soruIndeksi + 1}/${durum.kuyruk.length}`);
        io.emit('yeniTur', {});
        return { ok: true, bulmacaId: r.bulmaca.id, sira: durum.soruIndeksi + 1 };
      });
      T('t:iptal', () => {
      ...
      // misafir · öğrenci listesi · Etkinlik Bilgisi · ölçme/CSV (ayrıntı: lib/panelOlaylari.js)
      panelOlaylari(socket, T, durum, gunluk, yayinla);
