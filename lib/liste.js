...
const DONUSUM = path.join(__dirname, '..', 'data', 'kod_esleme.json');

/* Grup birleşmesi (İ+C → U) eski kodları: "C-04" → "U-24". Dosya yoksa boş tablo.
   İki biçim de okunur: { esleme: [{eski, yeni}] } (standart) ve { donusum: { eski: yeni } }. */
function donusumYukle() {
  try {
    const ham = JSON.parse(fs.readFileSync(DONUSUM, 'utf8'));
    if (Array.isArray(ham.esleme)) {
      const out = {};
      for (const e of ham.esleme) if (e && e.eski && e.yeni) out[String(e.eski).toUpperCase()] = String(e.yeni).toUpperCase();
      return out;
    }
    return ham.donusum || {};
  } catch (e) { return {}; }
...
class Liste {
  ...
  yukle() {
    ...
    this.kayitlar = (ham.ogrenciler || []).map((o) => ({
      ...
      aktif: o.aktif !== false,
      // birleşme öncesi kod (arşiv verisini bağlamak için) — KORUNUR, indirilen dosyaya da yazılır
      eski_kod: o.eski_kod ? String(o.eski_kod).toUpperCase() : undefined,
      misafir: false
  ...
  bul(kod) {
    ...
    return this.kayitlar.find((o) => o.kod === k) ||
      this.kayitlar.find((o) => o.eski_kod === k) ||
  ...
  json() {
    return JSON.stringify({
      ...
      ogrenciler: this.kayitlar
        .filter((o) => !o.misafir)
        .map(({ kod, isim, grup, aktif, eski_kod }) =>
