...
class Durum {
  constructor() {
    ...
    this.grup = bulmacalar.gruplar[0] || gruplar.GECERLI[0];
    ...
    this.kuyruk = [];                         // oturumun bulmaca sırası (iki ilerleme modunda da)
    this.soruIndeksi = 0;                     // senkron modda kuyruktaki etkin soru
    this.turSifirla();
  }
  ...
  katil(kod, sid) {
    ...
    kod = kayit.kod;                                  // eski kodla gelse de yeni kodla sürer
    ...
    const yeni = {
      ...
      cevaplananlar: new Set(),                 // bu oturumda cevabı gönderilen bulmacalar (tekrarda puan yok)
  ...
  /* ---------------- öğretmen ---------------- */
  /* ---------------- öğretmen: oturum akışı (ayrıntı: lib/oturum.js) ---------------- */
  etiketAyarla(metin) { return oturum.etiketAyarla(this, metin); }
  ayarla(ayarlar) { return oturum.ayarla(this, ayarlar); }
  basla(bulmacaId) { return oturum.basla(this, bulmacaId); }
  duraklat(deger) { return oturum.duraklat(this, deger); }
  bitir() { return oturum.bitir(this); }
  etkinligiBitir() { return oturum.etkinligiBitir(this); }
  ...
  /* Bireysel modda 90 sn takılan öğrencinin soruyu geçmesi (0 puan, kayda "atlandi"). */
  pasGec(sid) { return akis.pasGec(this, sid); }

  /* Soru atlama ve sıradaki soru (yalnız senkron) — ayrıntı: lib/oturum.js */
  soruAtla(hedef) { return oturum.soruAtla(this, hedef); }
  sonraki() { return oturum.sonraki(this); }
