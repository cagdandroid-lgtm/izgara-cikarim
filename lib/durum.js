...
class Durum {
  ...
  /* ---------------- oyuncular ---------------- */
  adNormal(ad) { return String(ad || '').trim().replace(/\s+/g, ' ').slice(0, 24); }

  katil(ad, sid) {
    ad = this.adNormal(ad);
    if (ad.length < 2) return { hata: 'Adın en az 2 harf olmalı.' };
    const kucuk = ad.toLocaleLowerCase('tr');

    const yasak = this.yasaklar.get(kucuk);
    if (yasak && yasak > simdi()) {
      return { hata: 'Öğretmenin seni oyundan çıkardı. Birkaç dakika sonra tekrar dene.' };
    }
    if (yasak) this.yasaklar.delete(kucuk);

    // 1) aynı oturum kimliği ile dönüş
    let o = sid ? this.oyuncular.get(sid) : null;
    // 2) aynı isimle dönüş (yeni tarayıcı/oturum)
    if (!o) {
      for (const p of this.oyuncular.values()) {
        if (p.ad.toLocaleLowerCase('tr') === kucuk) { o = p; break; }
      }
    }
    if (o) {
      o.ad = this.isimKilitli ? o.ad : ad;
      o.cevrimici = true;
      return { oyuncu: o, geriDondu: true };
    }
    if (this.girisKilitli) return { hata: 'Öğretmen yeni katılımları kapattı. 🔒' };
    if (this.isimKilitli) return { hata: 'İsimler kilitli. Daha önce girdiğin adı kullanmalısın.' };

    const yeni = {
      id: sid || ('o' + Math.random().toString(36).slice(2, 10)),
      ad, puan: 0, cevrimici: true,
  ...
  at(id) {
    ...
    this.yasaklar.set(o.ad.toLocaleLowerCase('tr'), simdi() + YASAK_MS);
    return o;
  }

  isimDegistir(id, ad) {
    const o = this.oyuncular.get(id);
    const yeni = this.adNormal(ad);
    if (!o || yeni.length < 2) return null;
    o.ad = yeni;
    return o;
  }
  ...
  kilit(tip, deger) {
    ...
    if (tip === 'isim') this.isimKilitli = !!deger;
  }
  ...
  kamu() {
    return {
      faz: this.faz, mod: this.mod, duraklatildi: this.duraklatildi,
      girisKilitli: this.girisKilitli, isimKilitli: this.isimKilitli,
      grup: this.grup, seviye: this.seviye, turNo: this.turNo,
      bulmaca: bulmacalar.acik(this.bulmaca),
      kalanSn: this.kalanSn(), sureSn: this.sureSn,
      skorlar: this.skorlar(),
      podyum: this.podyum.map((p) => ({ ad: p.ad, sure: p.sure, sira: p.sira, puan: p.puan }))
    };
  }
  ...
  kisisel(sid) {
    const o = this.oyuncular.get(sid);
    if (!o) return null;
    const t = this.mod === 'ikili' ? this.takim(o.id) : null;
    return {
      id: o.id, ad: o.ad, puan: o.puan, bitti: o.bitti, denemeler: o.denemeler,
      isaretler: this.isaretTablosu(o),
      sonSonuc: this.mod === 'birlikte' ? this.sonKontrol : (t ? t.sonSonuc : o.sonSonuc),
      takim: t ? {
        id: t.id, ad: this.takimAdi(t), bitti: t.bitti, denemeler: t.denemeler,
  ...
  ogretmenEk() {
    const oyuncular = [...this.oyuncular.values()].map((o) => {
      const ilr = this.bulmaca ? ilerleme(this.bulmaca, this.isaretTablosu(o)) : { yuzde: 0, kesin: 0 };
      const t = this.mod === 'ikili' ? this.takim(o.id) : null;
      return {
        id: o.id, ad: o.ad, puan: o.puan, cevrimici: o.cevrimici, bitti: o.bitti,
        denemeler: o.denemeler, ilerleme: ilr.yuzde, kesin: ilr.kesin,
        turPuani: this.turPuanlari[o.id] || 0,
        takimId: t ? t.id : null, takimAd: t ? this.takimAdi(t) : null
      };
    }).sort((a, b) => b.puan - a.puan || a.ad.localeCompare(b.ad, 'tr'));
    return {
      oyuncular,
      takimlar: this.takimOzet(),
      cozum: this.bulmaca ? this.bulmaca.cozum : null,
      olcum: this.olcum.panel(this.oyuncular),
