...
class Olcum {
  ...
  kod(oyuncu, oyuncular) {
    ...
    const kod = 'E-' + iki(++this.sayac);
    this.kodTablosu.set(oyuncu.id, { kod, ad: oyuncu.ad });
    return kod;
  }
  ...
  eslesme(oyuncular) {
    ...
    for (const [id, k] of this.kodTablosu) {
      ...
      out.push({ id, kod: k.kod, ad: o ? o.ad : k.ad, oyunda: !!o });
    }
  ...
  yaz(durum, oyuncu, { sonuc, sureMs, deneme }) {
    const b = durum.bulmaca;
  ...
  turKapat(durum) {
    ...
    for (const o of durum.oyuncular.values()) {
      ...
      this.yaz(durum, o, { sonuc: 'atlandi', sureMs: durum.gecenMs(), deneme: Math.max(1, o.denemeler) });
      n++;
    }
    return n;
  }
  ...
  csv({ isimli } = {}) {
    const alanlar = isimli ? [...SUTUNLAR, 'ogrenci_ad'] : SUTUNLAR;
    const adlar = new Map([...this.kodTablosu.values()].map((k) => [k.kod, k.ad]));
    const kacir = (v) => {
  ...
  panel(oyuncular) {
    const ozetler = {};
    for (const id of oyuncular.keys()) ozetler[id] = this.ozet(id);
    return {
...
module.exports = { Olcum, SUTUNLAR, OYUN };
