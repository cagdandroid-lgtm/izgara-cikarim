...
function dogrula(b, i) {
  ...
  // geçerli gruplar p, e, u — eski "i"/"c" değerleri "u" olarak kabul edilir
  const g = gruplar.normal(b.grup);
  if (!g) hata(`geçersiz grup "${b.grup}" (geçerli: ${gruplar.GECERLI.join(', ')})`);
  b.grup = g;
...
/* Öğrenciye giden kimlik OPAKTIR: gerçek kimlik ("c-2-09") katman kodunu içerir ve zorluk
   gizliliğini bozar. Aynı bulmaca daima aynı opak kimliği alır (istemci yeniden çizimi için). */
function opakKimlik(id) {
  let h = 2166136261;
  for (const c of String(id)) h = Math.imul(h ^ c.charCodeAt(0), 16777619) >>> 0;
  return 'b' + h.toString(36);
}

/* istemciye giden sürüm: çözüm YOK, zorluk YOK, gerçek kimlik YOK */
function acik(b) {
  if (!b) return null;
  // ZORLUK GİZLİLİĞİ: seviye/zorluk alanı öğrenci paketine KONMAZ (CLAUDE.md)
  return {
    id: opakKimlik(b.id), grup: b.grup, tema: b.tema, baslik: b.baslik,
...
// grup listesi resmî sırayla (p, e, u) ve yalnız içeriği olan gruplar
const icerikGruplari = gruplar.GECERLI.filter((g) => hepsi.some((b) => b.grup === g));

module.exports = {
  hepsi,
  gruplar: icerikGruplari,
  seviyeler,
  acik,
  bul: (id) => hepsi.find((b) => b.id === id) || null,
  liste: (grup, seviye) => {
    const g = grup ? gruplar.normal(grup) : null;          // eski "i"/"c" → "u"
    return hepsi.filter((b) => (!g || b.grup === g) && (!seviye || b.seviye === seviye));
  },
